...
function FocusSession({ TotalSessionMinute, setIsRun, mode, currentSessionMinute }) {
  ...
  return (
    <section className="flex flex-col justify-start items-center shadow-lg bg-gray-800 text-white p-8 rounded-lg  gap-4 select-none">
      <div>
        <h1 className="font-extrabold text-2xl md:text-3xl text-center">Focus here and now</h1>
      </div>
      <div className="grid grid-rows-2 grid-cols-3 bg-gray-700 rounded-lg w-1/3  md:w-3/7  p-4 gap-2">
        <div className="row-span-2 col-span-2 flex flex-col justify-between items-center border-r-2 border-gray-600 pr-2">
          <input
            type="text"
            inputMode="numeric"
            pattern="[0-9]*"
        ...
        <div className="flex justify-center items-center text-gray-800 text-3xl border-b-2 border-amber-100 pl-2">
          <ArrowUp onClick={incHandler} className="cursor-pointer hover:text-blue-500 transition-transform hover:scale-110" />
        </div>
        <div className="flex justify-center items-center text-gray-800 text-3xl border-amber-100 pl-2">
